...
public class Load
{
    ...
    public (List<Skill> skills, List<DailyHabit> habits) LoadSkillAndHabits()
    {
        List<Skill> skills = new List<Skill>();
        List<DailyHabit> habits = new List<DailyHabit>();
        List<string> habitLines = new List<string>();
        ...
        foreach (string line in lines)
        {
            ...
            if (parts[0] == "Skill")
            {
                skills.Add(Skill.Parse(line));
            }
            else if (parts[0] == "Habit")
            {
                habitLines.Add(line);
            }
        }

        foreach (string habitLine in habitLines)
        {
            habits.Add(DailyHabit.Parse(habitLine, skills));
        }

        Console.WriteLine("Skills and habits loaded successfully!");
        return (skills, habits);
    }
